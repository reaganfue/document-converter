...
| 頁面 | 功能 |
|------|------|
| 首頁 | 拖放檔案即時轉換（最多 20 檔，單檔 50MB） |
| 批次 | 整個資料夾批次轉換 |
| PDF 工具 | 合併 / 分割 / 壓縮 / 加密解密 / 點擊編輯 PDF 文字 |
| 圖片工具 | AI 自動去背（rembg）/ 背景替換 / 邊緣銳化羽化 |
| 歷史 | 轉換記錄查詢 + 一鍵重新轉換 |
...
- 掃描版 PDF 不支援 OCR（純圖片 PDF 轉文字結果為空）
...
1. 確保 `u2net_models/` 已存在（執行 `scripts\predownload_models.py`）
2. 雙擊 `build.bat`（自動清理舊產物 → PyInstaller 打包 → 驗證）
3. 產物在 `dist\文件轉檔\`，整個資料夾可複製到其他電腦
4. 雙擊 `文件轉檔.exe` 即可使用

注意：
- 產物約 1.1GB（含 Python runtime、所有依賴與 525MB AI 去背模型）
